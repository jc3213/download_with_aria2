document.querySelector('iframe').addEventListener('load', async event => {
    ...
    legacy.innerHTML = '<div><span class="title">Capture API</span> <select name="capture_api" value="1"><option value="0">downloads API</option><option value="1">webRequest API</option></select></div>\
    ...
    legacy.querySelectorAll('[name]').forEach(field => field.value = store[field.name] ?? field.value);
